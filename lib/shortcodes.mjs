 ...
 * <img> fallback, plus a <figcaption>. `imageMetadata` is in eleventy-img's
 * shape: `{ format: [{ url, width, height, sourceType, srcset }, ...] }`,
 * smallest first — whether it came from eleventy-img itself or from a set
 * manifest via photoVariants().
 *
...
/**
 * A photo resized at build time from a source file in the repo (`img/...`).
 * Being retired: new photos are published to the media bucket ahead of the
 * build and embedded with photoShortcode() instead.
 */
export const imageShortcode = async (
  ...
  return pictureHtml(imageMetadata, alt, className, sizes);
};

/**
 * A photo already published to the media bucket, referenced as
 * `<set-id>/<name>` — e.g. `Last-Ever-Last-Ever/IMG_9410`. Same output as
 * imageShortcode(), with every URL on `mediaUrl`.
 */
